...
ARIA (Automated Response & Incident Agent) is an AI-powered SRE pipeline that automates incident management for e-commerce platforms. When something breaks in production, engineers waste critical minutes reading logs, assigning priorities, and notifying the right people. ARIA eliminates that overhead by handling the entire workflow automatically — from report submission to team notification and resolution.
The agent uses Google Gemini with multimodal input (text, screenshots, log files) and fetches real TypeScript source files from the Medusa.js GitHub repository before each triage. This grounds every root cause hypothesis in actual production code rather than generic knowledge, producing structured output with priority level, severity score, recommended actions, and a runbook for the on-call engineer.
The full pipeline — ingest, triage, ticket creation, team notification via email and Discord, and reporter confirmation — runs end-to-end with a single docker compose up --build command, fully observable through a structured event log and real-time WebSocket stream in the UI.
